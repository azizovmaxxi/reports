...
exec rpt_2_2 '2018-01-01', '2018-12-31', 68241, 68241

GO
IF OBJECT_ID ( 'rpt_2_2', 'P' ) IS NOT NULL DROP PROCEDURE rpt_2_2;

GO
USE [person]
GO
/****** Object:  StoredProcedure [dbo].[rpt_2_2]    Script Date: 01.10.2020 15:31:07 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[rpt_2_2] @sdate date, @edate date, @lpu_id INT, @lpu_parent_id INT
AS
BEGIN
    ...
    ;WITH a_tmp_visit(case_id, visit_date, lpu_id, doctor_id, post_code, patient_id, birth_day, vistyp_code)
         ...
         result_data(doctor_id, post_code, lpu_id, a1, a2, a3, a4, a5, a6, a7, a8, a9)
             AS (
    ...
    SELECT lpu.parent_id, p_d.CODE AS doctor_code, r_data.lpu_id, lpu.code AS lpu_code, lpu.short_name,
           gsp.full_name AS post_name, gsp.code AS post_code, p_d.Name AS doctor_name,
           r_data.a1, r_data.a2, r_data.a3, r_data.a4, r_data.a5, r_data.a6, r_data.a7, r_data.a8, r_data.a9,
           CONCAT(lpu.short_name, ' (', (CASE WHEN parent_id IS NULL THEN 'Узкие специалисты' ELSE CONVERT(VARCHAR, lpu.code) END), ')') AS build_group_title
    ...
    ORDER BY parent_id ASC, lpu_id DESC, doctor_name ASC
