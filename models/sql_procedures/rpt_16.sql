...
CREATE PROCEDURE rpt_16 (@sdate date, @edate date, @lpu_id int, @lpu_parent_id int)
as set nocount on
	...
	set @sql =';with ACase as
		( 
			...
			where a.f_v_date >= @sdate and a.f_v_date <= @edate and a.recor = 1 
			and d.icd10 between ''I60'' and ''I649'' and ' + @where + '			 
		)
