...
--
-- execute rpt_30 @sdate, @edate, @lpu_id,@lpu_parent_id

GO
IF OBJECT_ID ( 'rpt_30', 'P' ) IS NOT NULL DROP PROCEDURE rpt_30;

GO
CREATE PROCEDURE rpt_30 @sdate date, @edate date, @lpu_id INT, @lpu_parent_id INT
AS
BEGIN
    ...
    DECLARE @result_rpt_30 NVARCHAR(MAX) = '
     WITH calc_result(c1,c2,c3,c4,c5,c6,c7)
        AS (
            SELECT COUNT(*) AS c1,
                   COUNT(CASE WHEN birth_day<DateAdd(yyyy, -18, f_v_date) AND a_p_r.cervix = 1 THEN 1 ELSE NULL END) c2,
                   COUNT(CASE WHEN birth_day BETWEEN DateAdd(yyyy, -21, f_v_date) AND DateAdd (yyyy, -18, f_v_date)
                              AND a_p_r.cervix = 1  THEN 1 ELSE NULL END) c3,
                   COUNT(CASE WHEN birth_day BETWEEN DateAdd(yyyy, -31, f_v_date) AND DateAdd (yyyy, -21, f_v_date)
                              AND a_p_r.cervix = 1 THEN 1 ELSE NULL END) c4,
                   COUNT(CASE WHEN birth_day BETWEEN DateAdd(yyyy, -41, f_v_date) AND DateAdd (yyyy, -31, f_v_date)
                              AND a_p_r.cervix = 1 THEN 1 ELSE NULL END) c5,
                   COUNT(CASE WHEN birth_day BETWEEN DateAdd(yyyy, -50, f_v_date) AND DateAdd (yyyy, -41, f_v_date)
                              AND a_p_r.cervix = 1 THEN 1 ELSE NULL END) c6,
                   COUNT(CASE WHEN birth_day<DateAdd(yyyy, -50, f_v_date) AND a_p_r.cervix = 1 THEN 1 ELSE NULL END) c7
            FROM A_case AS a_c
    ...
    EXECUTE sp_executesql @result_rpt_30, N'@sdate date, @edate date, @lpu_id INT, @lpu_parent_id INT',
