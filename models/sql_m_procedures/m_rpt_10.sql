...
GO
IF OBJECT_ID ( 'm_rpt_10', 'P' ) IS NOT NULL DROP PROCEDURE m_rpt_10;

GO
CREATE PROCEDURE m_rpt_10 @sdate date, @edate date, @lpu_id INT, @lpu_parent_id INT
AS
BEGIN
    ...
    DECLARE @result_m_rpt_10 NVARCHAR(MAX) = '
       SELECT count(*) c1
       FROM M_case
       INNER JOIN glb_lpu lpu ON lpu.id = M_case.lpu_id
       WHERE direction_dortor = 1
          AND f_v_date BETWEEN @sdate AND @edate
          AND ('+ @where_lpu +')
    '

    EXECUTE sp_executesql @result_m_rpt_10, N'@sdate date, @edate date, @lpu_id INT, @lpu_parent_id INT',
